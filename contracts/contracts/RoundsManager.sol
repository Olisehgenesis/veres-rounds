...
contract RoundManager is ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => address)) public roundBeneficiaries; // saccoId => roundNumber => beneficiary
    mapping(uint256 => address[]) public saccoParticipantsQueue; // Queue of members waiting for their turn
        ...
        roundActive(roundId) 
    {
        ...
        // Register contribution in SaccoManager
        saccoManager.registerContribution(round.saccoId, msg.sender, round.requiredContribution);
    ...
    /**
     * @dev Sets up the initial beneficiary queue for a SACCO
     */
    function setupBeneficiaryQueue(uint256 saccoId) 
        external 
        onlySaccoAdmin(saccoId) 
    {
        // Check if queue already exists
        require(saccoParticipantsQueue[saccoId].length == 0, "Queue already set up");
        
        // Get all SACCO members
        address[] memory members = saccoManager.getSaccoMembers(saccoId);
        
        // Add members to queue
        for (uint256 i = 0; i < members.length; i++) {
            saccoParticipantsQueue[saccoId].push(members[i]);
        }
    }

    /**
     * @dev Adds a member to the beneficiary queue
     */
    function addToQueue(uint256 saccoId, address member) 
        external 
        onlySaccoAdmin(saccoId) 
    {
        require(saccoManager.isMember(saccoId, member), "Not a SACCO member");
        
        // Check if already in queue
        bool alreadyInQueue = false;
        for (uint256 i = 0; i < saccoParticipantsQueue[saccoId].length; i++) {
            if (saccoParticipantsQueue[saccoId][i] == member) {
                alreadyInQueue = true;
                break;
            }
        }
        
        require(!alreadyInQueue, "Already in queue");
        
        saccoParticipantsQueue[saccoId].push(member);
    }

    /**
     * @dev Gets the next beneficiary from the queue
     */
    function getNextBeneficiary(uint256 saccoId) 
        external 
        view 
        returns (address) 
    {
        require(saccoParticipantsQueue[saccoId].length > 0, "Queue is empty");
        
        return saccoParticipantsQueue[saccoId][0];
    }

    /**
     * @dev Advances the queue to the next beneficiary
     */
    function advanceQueue(uint256 saccoId) 
        external 
        onlySaccoAdmin(saccoId) 
    {
        require(saccoParticipantsQueue[saccoId].length > 0, "Queue is empty");
        
        // Save the current beneficiary
        address currentBeneficiary = saccoParticipantsQueue[saccoId][0];
        
        // Remove from front of queue
        for (uint256 i = 0; i < saccoParticipantsQueue[saccoId].length - 1; i++) {
            saccoParticipantsQueue[saccoId][i] = saccoParticipantsQueue[saccoId][i + 1];
        }
        
        // Add to end of queue (circular queue)
        saccoParticipantsQueue[saccoId][saccoParticipantsQueue[saccoId].length - 1] = currentBeneficiary;
    }

    /**
     * @dev Assigns the next beneficiary automatically based on the queue
     */
    function assignNextBeneficiary(uint256 roundId) 
        external 
        roundExists(roundId) 
        onlySaccoAdmin(rounds[roundId].saccoId) 
    {
        ...
        uint256 saccoId = round.saccoId;
        require(saccoParticipantsQueue[saccoId].length > 0, "Queue is empty");
        
        // Get next beneficiary from queue
        address nextBeneficiary = saccoParticipantsQueue[saccoId][0];
        
        // Set as beneficiary
        round.beneficiary = nextBeneficiary;
        roundBeneficiaries[saccoId][round.roundId] = nextBeneficiary;
        
        emit BeneficiaryChanged(roundId, nextBeneficiary);
        
        // Advance queue for next round
        this.advanceQueue(saccoId);
    }

    /**
     * @dev Automatically start a new round with the next beneficiary in the queue
     */
    function autoStartNextRound(
        ...
        external 
        onlySaccoAdmin(saccoId) 
        returns (uint256 newRoundId) 
    {
        // Create new round
        this.createRound(saccoId, requiredContribution, targetAmount, durationInDays);
        uint256 roundId = roundCount;
        
        // Assign next beneficiary
        this.assignNextBeneficiary(roundId);
        
        // Start round
        this.startRound(roundId, rounds[roundId].beneficiary, durationInDays);
        
        // Advance the SACCO's round counter
        saccoManager.advanceRound(saccoId);
        
        return roundId;
    }

    /**
     * @dev Gets the full beneficiary queue for a SACCO
     */
    function getBeneficiaryQueue(uint256 saccoId) 
        external 
        view 
        returns (address[] memory) 
    {
        return saccoParticipantsQueue[saccoId];
    }

    /**
     * @dev Shuffles the beneficiary queue randomly
     * Note: This is not cryptographically secure but sufficient for this use case
     */
    function shuffleQueue(uint256 saccoId) 
        external 
        onlySaccoAdmin(saccoId) 
    {
        address[] storage queue = saccoParticipantsQueue[saccoId];
        uint256 length = queue.length;
        
        for (uint256 i = 0; i < length; i++) {
            uint256 j = i + uint256(keccak256(abi.encodePacked(block.timestamp, block.prevrandao, i))) % (length - i);
